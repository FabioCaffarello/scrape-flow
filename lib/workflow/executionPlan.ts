...
export function FlowToExecutionPlan(
  ...
  // Initialize the execution plan with the entry point
...
function getInvalidInputs(node: AppNode, edges: Edge[], planned: Set<string>) {
  ...
  for (const input of inputs) {
    ...
    // If a input is not provided by the user then we need to check
    ...
    if (requiredInputProvidedByVisitedOutput) {
      ...
      if (!inputLinkedToOutput) {
        // no output linked to this input, so we can move on
        continue;
      }

      if (inputLinkedToOutput && planned.has(inputLinkedToOutput.source)) {
        // The output is proding a value to the input
        continue;
      }
    }
